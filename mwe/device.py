...
from bacpypes.object import AnalogValueObject, BinaryValueObject, FileObject, FileAccessMethod, BinaryInputObject
...
class SampleApplication(
    BIPSimpleApplication, ReadWritePropertyMultipleServices, ChangeOfValueServices, FileServices
):
    pass
...
class TestStreamFileObject(LocalStreamAccessFileObject):
    ...
    def read_stream(self, start_position, octet_count):
        print('Request received, reading', flush=True)
        with open(self._filename, 'rb') as fin:
            pos = fin.seek(start_position, 0)
            print('Located starting position at', pos, flush=True)
            data = fin.read(octet_count)
        print('Read all data, returning', flush=True)
        return False, data
...
def main():
    ...
    for i in range(20):
        trash_bi = BinaryInputObject(
            objectIdentifier=("binaryInput", i),
            objectName="bi" + str(i),
            presentValue="inactive",
            description="Trash, should be skipped",
            statusFlags=[0, 0, 0, 0]
        )
        test_application.add_object(trash_bi)
        trash_bv = BinaryValueObject(
            objectIdentifier=("binaryValue", i + 20),
            objectName="bv" + str(i),
            presentValue="inactive",
            description="Trash, but should not be skipped",
            statusFlags=[0, 0, 0, 0]
        )
        test_application.add_object(trash_bv)
    
    _log.debug("    - object list: %r", this_device.objectList)
    
    #test_record_file = TestRecordFileObject('test.txt')
    #test_application.add_object(test_record_file)
    test_stream_file = TestStreamFileObject('file.pdf')
